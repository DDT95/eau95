...
const sources = [
  ...
  {id:"sup-as1",title:"SUP AS1 · protection des eaux",date:"référentiel national",dataset:"sup-as1-perimetres-de-protections-liees-aux-servitudes-de-la-categorie-as1-protection-des-eaux-1",file:null,group:"Servitudes",color:"#000091",kind:"unavailable",count:"Fiche DDT 95 référencée",active:false},
  {id:"sup-a5",title:"SUP A5 · canalisations d’eau et d’assainissement",date:"référentiel national",url:"https://www.geoportail-urbanisme.gouv.fr/services/",file:null,group:"Servitudes",color:"#6a6af4",kind:"unavailable",count:"Source Géoportail de l’urbanisme",active:false,producer:"Géoportail de l’urbanisme"},
  ...
  {id:"hydro-live",title:"Stations hydrométriques",date:"temps quasi réel",api:"https://hubeau.eaufrance.fr/api/v2/hydrometrie/referentiel/stations?code_departement=95&size=200&format=json",group:"Hub’Eau · mesures",color:"#0078f3",kind:"api-point",count:"17 stations",active:false,producer:"Hub’Eau · Vigicrues"},
  {id:"river-live",title:"Stations de qualité des rivières",date:"référentiel national",api:"https://hubeau.eaufrance.fr/api/v2/qualite_rivieres/station_pc?code_departement=95&size=500&format=json",group:"Hub’Eau · mesures",color:"#009081",kind:"api-point",count:"56 stations",active:false,producer:"Hub’Eau · Naïades"},
  {id:"groundwater",title:"Points de qualité des nappes",date:"synchronisation continue",api:"https://hubeau.eaufrance.fr/api/v1/qualite_nappes/stations?num_departement=95&size=500&format=json",group:"Hub’Eau · mesures",color:"#6a4c93",kind:"api-point",count:"451 points",active:false,producer:"Hub’Eau · ADES"},
  {id:"fish",title:"Stations d’inventaires piscicoles",date:"plus de 50 ans d’observations",api:"https://hubeau.eaufrance.fr/api/v1/etat_piscicole/stations?code_departement=95&size=200",group:"Biodiversité",color:"#18753c",kind:"api-point",count:"38 stations",active:false,producer:"Hub’Eau · OFB / ASPE"},
  {id:"hydrobio",title:"Stations hydrobiologiques",date:"référentiel Naïades",api:"https://hubeau.eaufrance.fr/api/v1/hydrobio/stations_hydrobio?code_departement=95&size=200",group:"Biodiversité",color:"#009081",kind:"api-point",count:"59 stations",active:false,producer:"Hub’Eau · Naïades"},
...
function displayTitle(source,p){
  return p.libelle_station||p.libelle_station_hydrobio||p.libelle_pe||p.LbStationM||p.NOM_SI||p.DONNEES__1||p.LbZAS||p.name||p.Nom||p.NOM||p.NomCoursdE||p["POLICE EAU"]||source.title;
}
function relevantProps(source,p){
  const preferred={stations:["CdStationM","LbStationM","NomCoursdE","LbCommune","pH","Temp","Conducti","DatePrelev"],cours:["NOM","ETAT","ZNT","BCAE","ORIGINE1"],qualite:["name","etateco","annee","longueur","statut"],prix:["NOM","INSEE_COM","TARIF_AEP","POPULATION"],secheresse:["LbZAS","TypeZAS","MnZAS"],"syndicats-eau":["NOM","NOM_SI","SIREN_SI","INSEE_COM"],"syndicats-assain":["NOM","DONNEES__1","DONNEES_SI","INSEE_COM"],"syndicats-riviere":["NOM","NB_Syndic","Syndic_RIV","SI_RIV","SI_RIV_2","SI_RIV_3","INSEE_COM","SIREN_EPCI"],police:["NOM","POLICE_EAU","NOM_2","ID_COURSDE"],"hydro-live":["code_station","libelle_cours_eau","libelle_commune","en_service","date_ouverture_station","date_fermeture_station","qualification_donnees_station"],"river-live":["code_station","nom_cours_eau","libelle_commune","nom_masse_deau","localisation_precise","date_creation","date_arret"],groundwater:["bss_id","code_bss","nom_commune","nom_nature_pe","nom_etat_pe","date_debut_mesure","date_fin_mesure","profondeur_investigation","noms_masse_eau_edl"]};
...
function openFeature(source,feature){
  ...
  document.getElementById("detailContent").innerHTML=`<span class="detail-tag">${source.group} · ${source.producer||"DDT 95"}</span><h2>${safe(title)}</h2><p class="subtitle">${source.title} · ${source.date}</p><div class="property-grid">${cards}</div><div id="liveDetail"></div><a class="source-link" target="_blank" rel="noopener" href="${externalSourceUrl(source)}">Consulter la source officielle ↗</a>`;
...
async function init(){renderLayers();renderSources();await Promise.all(sources.filter(s=>s.file).map(loadSource));await Promise.all(sources.filter(s=>s.active).map(s=>setLayer(s,true)));await loadCommunes();document.getElementById("mapStatus").textContent="10 couches DDT 95 prêtes · 6 connexions Hub’Eau disponibles"}
